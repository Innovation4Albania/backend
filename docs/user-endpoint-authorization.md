...
## Shenime per zhvillime te ardhshme

- Mos perdor `CanManageUsers` per lexime qe sherbejne vetem si reference ne UI; perdor `CanReadManagedUsers`.
- Mos lejo veprime mutimi me `CanReadManagedUsers`.
- Cdo endpoint i ri qe krijon, ndryshon ose fshin llogari duhet te perdore `CanManageUsers`.
- Cdo ndryshim ne role duhet te perditesoje kete dokument dhe testet ne `ApplicationRolesTests`.
