...
public interface IInnovationDashboardRepository
{
    ...
    Task<(bool IsSuccess, ProgramMetricsResponse? Response, string? Error)> TryUpdateProgramMetricsAsync(UserContext context, string programKey, UpdateProgramMetricsRequest request);
    ...
    Task<(bool IsSuccess, ProjectChangeProposalResponse? Response, string? Error)> TryResolveChangeProposalAsync(UserContext context, string id, string action, string? resolutionReason = null);
    Task<(bool IsSuccess, string? Error)> TryDeleteChangeProposalAsync(UserContext context, string id);
