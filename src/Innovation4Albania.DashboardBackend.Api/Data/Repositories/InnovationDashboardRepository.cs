...
public sealed class InnovationDashboardRepository(InnovationDashboardStore store) : IInnovationDashboardRepository
{
    ...
    public Task<ProgramMetricsResponse?> GetProgramMetrics(string programKey) => store.GetProgramMetrics(programKey);
    public Task<(bool IsSuccess, ProgramMetricsResponse? Response, string? Error)> TryUpdateProgramMetricsAsync(UserContext context, string programKey, UpdateProgramMetricsRequest request) => store.TryUpdateProgramMetricsAsync(context, programKey, request);
