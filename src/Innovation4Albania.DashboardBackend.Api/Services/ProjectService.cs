...
{
    public Task<IReadOnlyList<ProjectResponse>> GetProjects(UserContext context, string? status, string? query) => repository.GetProjects(context, status, query);
    public Task<ProjectResponse?> GetProjectById(string id, UserContext context) => repository.GetProjectById(id, context);
